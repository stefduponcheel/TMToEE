...
def submit_jobs(test=False, jobid=None):
    ...
    # confirmed reachable from a worker node.
    outdir = f"/eos/user/s/sduponch/PhD/ppToTMeeX/GENSIM/{year}"
    ...
    subprocess.run(["./mkconfig_ppToTMeeX.sh"])
...
executable = /bin/bash
arguments = "{here}/run_ppToTMeeX.sh $(jobid) {events_per_job} {outdir}"
