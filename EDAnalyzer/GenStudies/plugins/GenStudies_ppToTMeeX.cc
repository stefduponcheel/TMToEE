...
namespace {
  constexpr int kTmPdgId = 99033003;
}

class GenStudies_ppToTMeeX : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit GenStudies_ppToTMeeX(const edm::ParameterSet&);
  ~GenStudies_ppToTMeeX() override = default;
...
private:
  ...
  // TM (last copy in the record -- the one that actually decays; earlier
  // copies are shower/recoil bookkeeping and point to the next TM copy,
  // not to e+e-)
...
GenStudies_ppToTMeeX::GenStudies_ppToTMeeX(const edm::ParameterSet& iConfig)
  : genParticlesToken_(consumes<std::vector<reco::GenParticle>>(
        iConfig.getParameter<edm::InputTag>("genParticles"))) {
  usesResource("TFileService");
}

void GenStudies_ppToTMeeX::beginJob() {
  edm::Service<TFileService> fs;
  tree_ = fs->make<TTree>("Events", "pp -> TM X -> ee X");
...
const reco::Candidate* GenStudies_ppToTMeeX::findDaughter(const reco::Candidate* mom, int pdgId) const {
...
void GenStudies_ppToTMeeX::analyze(const edm::Event& iEvent, const edm::EventSetup&) {
  edm::Handle<std::vector<reco::GenParticle>> genParticles;
  iEvent.getByToken(genParticlesToken_, genParticles);

  // TM is produced directly in the hard process (no single parent resonance
  // like eta/B/D/omega in the other channels), and typically appears as
  // several successive copies from shower/recoil bookkeeping. Keep
  // overwriting so the last one found is the pre-decay copy.
  const reco::Candidate* tm = nullptr;
  for (const auto& p : *genParticles) {
    if (p.pdgId() == kTmPdgId) tm = &p;
  }
  if (!tm) return;
...
void GenStudies_ppToTMeeX::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<edm::InputTag>("genParticles", edm::InputTag("genParticles"));
  descriptions.add("genStudies_ppToTMeeX", desc);
}

DEFINE_FWK_MODULE(GenStudies_ppToTMeeX);
